...
# Create directories for the cropped and resized images and annotations
os.makedirs('resized_cropped/images', exist_ok=True)
os.makedirs('resized_cropped/annotations', exist_ok=True)
...
for img_file in image_files[:subset_size]:
    ...
    for box in boxes:
        ...
        if box_area < image_area * 0.15:
            ...
            resized_cropped_img = cropped_img.resize((644, 644), Image.Resampling.LANCZOS)
            resized_cropped_img_path = f'resized_cropped/images/{img_file[:-4]}_{int(class_id)}_cropped_resized.jpg'
            resized_cropped_img.save(resized_cropped_img_path)

            # Save the new normalized annotation file
            resized_cropped_ann_path = f'resized_cropped/annotations/{img_file[:-4]}_{int(class_id)}_cropped_resized.txt'
            save_normalized_annotations(resized_cropped_ann_path, class_id)

print("Cropping, resizing, and annotation normalization complete!")
